                  ...
                  tabPanel(title = "Data",
                           fluidRow(column(2, h2("Data set up"),
                                           ...
                                           p(fileInput("alphadiv", label = "Upload Alpha Diversity", 
                                                       multiple = FALSE, accept = ".rds")),
                                           p(fileInput("propFilt", label = "Upload Beta Diversity (propFilt)", 
                                                       multiple = FALSE, accept = ".rds")),
                                           p(fileInput("brayFilt", label = "Upload Beta Diversity (brayFilt)", 
                                                        multiple = FALSE, accept = ".rds")),
                                    ...
                                    column(5, h3("How to upload your data"),"Text Instructions",
                                           tags$ol(
                                             ...
                                             'sampleID', 'individualID', 'Timepoint', 'Group', 'Condition'."), 
                                             tags$li("Once uploaded, the metadata table will be shown on the right and can be interactively browsed using the 
                                                     headers and search functions."), 
                                             tags$li("The phyloseq file will be summarised below, including information on min/max reads and simple metadata information."),
                                             ...
                                             tags$li("Beta diversity can be in the form of bray-curtis or jaccard metrics. This should be uploaded in two separate files; 
                                                     propFilt is a count matrix created using transform_sample_counts(), brayFilt is an ordination
                                                     of this propFilt object. Both files are required to create the beta diversity plot with plot_ordination()"),
...
server <- function(input, output) {
  ...
  output$alphadivPlot <- renderPlot({
    ...
    if (input$alphadivGeom == "jitter") {
      shannon %>%
        ...
        p 
      } else if (input$alphadivGeom == "boxplot") {
      shannon %>%
        ggplot(aes_string(x = input$group,
                          y = "shannon_entropy",
                          fill = input$colour)) +
  ...
  output$BFref_ui <- renderUI({
    ...
    Timepoints <- metadata$Timepoint %>% unique() %>% as.character()
    Conditions <- metadata$Condition %>% unique() %>% as.character()
    Refs <- c(Groups, Timepoints, Conditions) 
    
    selectInput("BFRefGroup", "Reference group",
                choices = Refs)
  ...
  output$BFsubsetVar_ui <- renderUI({
    ...
    if (input$BFsubsetTick == "TRUE") {
      bfratio %>% dplyr::filter(input$BFsubsetCol == input$BFsubset) %>%
    ...
    } else if (input$BFsubsetTick == "FALSE") {
        bfratio %>% 
          ggplot(aes_string(x = input$BFgroup, y = "bfratio",
                            fill = input$BFcolour)) + 
          geom_boxplot() +
